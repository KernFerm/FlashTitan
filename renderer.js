const state = {
  ...
  latestFlashResult: null,
  latestFailure: "",
  latestRecoveryGuidance: "",
  presets: [],
  selectedPreset: null
};
...
const elements = {
  ...
  appMetaPanel: document.getElementById("appMetaPanel"),
...
function formatModeLabel(mode) {
  ...
  if (normalized === "windows-installer-usb") {
    return "Windows installer USB";
  }
  if (normalized === "raw-write") {
    return "Direct raw write";
  }
  return normalized || "Unknown mode";
}

function buildVerificationSummary(result) {
  if (!result || typeof result !== "object") {
    return "Verification details are not available yet.";
  }

  if (result.mode === "windows-installer-usb") {
    const verifyMode = sanitizeText(result.verifyMode || state.settings.verificationMode || "full");
    return result.usedSplitImage
      ? `Windows installer workflow finished with ${verifyMode} verification, and the large installer image was split into FAT32-safe files.`
      : `Windows installer workflow finished with ${verifyMode} verification, and the boot files were staged for BIOS and UEFI startup.`;
  }

  const verificationReport = result.verificationReport || null;
  if (verificationReport?.mode === "quick") {
    return "Quick verification sampled the written media without finding a mismatch.";
  }
  if (verificationReport?.mode === "full") {
    return "Full verification compared the written media against the source image without finding a mismatch.";
  }

  return `FlashTitan finished using ${formatModeLabel(result.mode)}.`;
}

function buildDiagnosticsSnapshot() {
  const selectedDevices = getSelectedDevices();
  const latestResults = Array.isArray(state.latestFlashResult?.result) ? state.latestFlashResult.result : [];
  const resultLines = latestResults.length
    ? latestResults.map((entry) => {
        const targetName = sanitizeText(entry?.target?.name || entry?.target?.path || "Unknown target");
        const mode = formatModeLabel(entry?.result?.mode || "");
        const verifyMode = sanitizeText(entry?.result?.verifyMode || state.settings.verificationMode || "unknown");
        const extra = entry?.result?.usedSplitImage ? "split-image=yes" : "";
        return `- ${targetName}: mode=${mode}; verify=${verifyMode}${extra ? `; ${extra}` : ""}`;
      })
    : ["- No completed flash results in this session yet."];

  return [
    "FlashTitan diagnostics snapshot",
    `Time: ${new Date().toISOString()}`,
    `Image: ${sanitizeText(state.image?.fileName || "None selected")}`,
    `Image path: ${sanitizeText(state.image?.path || "Unavailable")}`,
    `Image route: ${sanitizeText(state.latestClassification?.mode || state.image?.classification?.mode || "Unknown")}`,
    `Verification setting: ${sanitizeText(state.settings.verificationMode || "unknown")}`,
    `Performance setting: ${sanitizeText(state.settings.performanceProfile || "unknown")}`,
    `Auto hash: ${state.autoHash ? "on" : "off"}`,
    `Selected targets: ${selectedDevices.length ? selectedDevices.map((device) => sanitizeText(device.name)).join(", ") : "None"}`,
    `Latest guidance: ${sanitizeText(state.latestGuidance || "None")}`,
    `Latest failure: ${sanitizeText(state.latestFailure || "None")}`,
    `Latest recovery guidance: ${sanitizeText(state.latestRecoveryGuidance || "None")}`,
    "Latest results:",
    ...resultLines
  ].join("\n");
}
...
function canStartFlash() {
  return Boolean(state.image && getSelectedDevices().length > 0 && !state.running);
}

function updateStartButtonState() {
  const ready = canStartFlash();
  const selectedDevice = getPrimarySelectedDevice();

  elements.startFlashButton.disabled = Boolean(state.running);
  elements.startFlashButton.textContent = state.running ? "Flashing..." : "Flash!";
  elements.startFlashButton.classList.toggle("primary-stage-button", ready && !state.running);
  elements.startFlashButton.classList.toggle("neutral-stage-button", !ready || state.running);

  if (state.running) {
    elements.refreshDevicesButton.textContent = "USB Locked";
  } else if (selectedDevice) {
    elements.refreshDevicesButton.textContent = "Target Selected";
  } else if (state.refreshingDevices) {
    elements.refreshDevicesButton.textContent = "Looking for Drives...";
  } else {
    elements.refreshDevicesButton.textContent = "Refresh USB Drives";
  }
}
...
function renderAppMeta() {
  if (!elements.appMetaPanel) {
    return;
  }

  const meta = typeof api?.getAppMeta === "function" ? api.getAppMeta() : null;
  const version = sanitizeText(meta?.version || "unknown");
  const platform = sanitizeText(meta?.platform || "unknown");
  const arch = sanitizeText(meta?.arch || "unknown");

  elements.appMetaPanel.innerHTML = `
    <strong>${escapeHtml(sanitizeText(meta?.name || "FlashTitan"))} ${escapeHtml(version)}</strong><br />
    <span>Runtime: ${escapeHtml(platform)} / ${escapeHtml(arch)}</span><br />
    <span>Public-use basics: run as Administrator, use removable media only, and verify on real hardware before broad release claims.</span><br />
    <span>Before calling this production-ready, keep the tested-image list, known limitations, and real-device results current.</span>
  `;
}
...
function renderImageSummary() {
  ...
  elements.imageSummary.innerHTML = `
    ...
    <span>Image size: ${escapeHtml(state.image.sizeLabel)} - Format: ${escapeHtml(state.image.extension)}</span><br />
    <span>${escapeHtml(state.image.sha256 ? "Image check finished." : "Checking the image now.")}</span><br />
...
function renderSafetyReview() {
  ...
  elements.safetyReview.innerHTML = `
    <strong>Ready to make your bootable USB</strong><br />
    <span>Selected USB drive: ${escapeHtml(primaryDevice.name)}</span><br />
    <span>USB size: ${escapeHtml(primaryDevice.sizeLabel)} - Drive letter: ${escapeHtml(primaryDevice.mountPoints?.join(", ") || "None")}</span><br />
    <span>Final check: ${escapeHtml(state.settings.verificationMode)} verification after writing.</span><br />
    <span>Everything currently on this USB drive will be erased.</span><br />
    <span>${escapeHtml(checksumLine)}</span><br />
    ${warningMarkup}
    <span>${
      "Image and target are ready. FlashTitan can start as soon as you press Flash."
    }</span>
  `;

  elements.statusBadge.textContent = "Ready to go";
  updateStartButtonState();
}
    ...
    .map((device, index) => {
      ...
      const warningMarkup =
        Array.isArray(device.warnings) && device.warnings.length
          ? `<div class="device-meta warning-text">Please double-check this device: ${escapeHtml(device.warnings.join(" | "))}</div>`
          : "";
      const advancedMeta =
        state.settings.experienceMode === "advanced"
          ? `<div class="device-meta">Technical details: Serial ${escapeHtml(device.serialNumber || "Unavailable")} - Partition ${escapeHtml(
              device.partitionStyle || "Unknown"
            )} - Health ${escapeHtml(device.healthStatus || "Unknown")} - Path ${escapeHtml(device.path)}</div>`
          : "";
      const labelLine = Array.isArray(device.volumeLabels) && device.volumeLabels.length
        ? `Label: ${escapeHtml(device.volumeLabels.join(", "))}`
        : "Label: None";
      const mountLine = device.mountPoints?.length
        ? `Drive letter: ${escapeHtml(device.mountPoints.join(", "))}`
        : "Drive letter: Not assigned";
      return `
        <article class="device-row ${selected ? "selected" : ""} ${device.blocked ? "blocked" : ""}">
          <div class="device-row-header">
            <div>
              <strong>${escapeHtml(device.name || "Removable USB drive")}</strong>
              <div class="device-meta">Size: ${escapeHtml(device.sizeLabel)} - Type: ${escapeHtml(device.type || "USB drive")}</div>
              <div class="device-meta">${mountLine} - ${labelLine}</div>
              ${advancedMeta}
            </div>
          ...
          <div class="device-actions">
            <span class="device-meta">${
              device.system
                  ? "This is a protected system drive."
                : selected
                  ? "This USB drive is selected."
                  : "This looks like a safe removable USB drive."
            }</span>
            <button class="secondary-button device-select-button" ${device.blocked ? "disabled" : ""}>
              ${selected ? "Selected USB Drive" : "Use This USB Drive"}
            </button>
          </div>
        </article>
      `;
...
async function refreshDevices() {
  ...
  try {
    ...
    if (state.selectedDeviceIds.length === 0) {
      const selectableDevices = state.devices.filter((device) => !device.blocked);
      if (selectableDevices.length === 1) {
        state.selectedDeviceIds = [sanitizeText(selectableDevices[0].id)];
      }
    }
    renderDevices();
    renderBadges(getPrimarySelectedDevice());
    ...
    appendLog(
      state.devices.length
        ? `Found ${state.devices.length} removable drive${state.devices.length === 1 ? "" : "s"}${state.selectedDeviceIds.length === 1 ? " and selected it automatically" : ""}.`
        : "No removable drives showed up yet."
    );
  } catch (error) {
...
async function copyDiagnostics() {
  try {
    await window.flashTitanApi.copyText(buildDiagnosticsSnapshot());
    appendLog("Diagnostics snapshot copied to the clipboard.", "success");
  } catch (error) {
    appendLog(error.message || "Failed to copy diagnostics.", "error");
  }
}
...
async function startFlash() {
  ...
  try {
    ...
    state.latestFlashResult = result;
    ...
    const verificationSummary = results.length
      ? results.map((entry) => buildVerificationSummary(entry.result)).join(" ")
      : `FlashTitan completed with ${sanitizeText(state.settings.verificationMode)} verification.`;
    state.latestFailure = "";
    state.latestRecoveryGuidance = "";
    ...
    elements.completionPanel.innerHTML = `
      ...
      <span>Verification result: ${escapeHtml(verificationSummary)}</span><br />
    ...
    state.latestFailure = sanitizeText(error.message || "Operation failed.");
  ...
  window.flashTitanApi.onFlashEvent((event) => {
    ...
    if (event.type === "classification") {
      ...
      state.latestClassification = {
        ...
        guidance: event.details?.preparedPath ? `Working file: ${event.details.preparedPath}` : state.latestGuidance,
    ...
    if (event.type === "warning") {
      state.latestFailure = sanitizeText(event.message || state.latestFailure);
    ...
    if (event.type === "guidance") {
      ...
      if (event.stage === "recovery") {
        state.latestRecoveryGuidance = state.latestGuidance;
      }
    ...
    if (event.type === "error") {
      state.latestFailure = sanitizeText(event.message || "Operation failed.");
