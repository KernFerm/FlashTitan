...
function extractMismatchOffset(message) {
...
function normalizeOperationError(error, context = {}) {
  ...
  if (lowered.includes("verification failed") || lowered.includes("mismatch between the source image")) {
    const mismatchOffset = extractMismatchOffset(message);
    return new Error(
      mismatchOffset === null
        ...
        : `FlashTitan finished writing, but the check back from ${targetName} found a mismatch at byte ${mismatchOffset}. Reconnect the drive, try a different USB port or removable drive, and run the flash again.`
    );
  }
