...
function buildRecoveryGuidance(message, targetCount) {
  ...
  const mismatchOffset = String(message || "").match(/byte\s+(\d+)/i)?.[1] || "";
  ...
  if (lowered.includes("did not match the source image")) {
    return mismatchOffset
      ? `Retry steps: try a different USB port or removable drive, then run the write again. If it fails again around byte ${mismatchOffset}, the removable media is likely unreliable.`
      : "Retry steps: try a different USB port or removable drive, then run the write again.";
  }
...
async function beginFlash(request) {
  ...
  try {
    ...
    const verificationSummary = results.map((entry) => ({
      targetName: entry.target?.name || entry.target?.path || "Unknown target",
