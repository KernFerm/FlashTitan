...
function classifyIsoFromInspection(prepared, inspection, hybrid) {
  ...
  const hasBootSignals = Boolean(inspection && (inspection.supportsBiosBoot || inspection.supportsUefiBoot));
  ...
  if (hasBootSignals) {
    return {
      mode: "raw-write",
      kind: "generic-bootable-iso",
      message:
        "Bootable ISO detected. FlashTitan will write the ISO directly to the target device even though it does not match a known Windows or Linux layout.",
      prepared,
      inspection,
      guidance: "Insert the USB into the target PC, open its boot menu, and choose the flashed drive to start the installer or recovery environment."
    };
  }
  ...
  return {
    ...
    message:
      "This ISO does not expose recognizable BIOS, UEFI, Windows installer, Linux, or hybrid boot signals, so FlashTitan cannot make it bootable safely yet.",
    prepared
  };
}
